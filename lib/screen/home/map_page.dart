...
class _MapPageState extends State<MapPage> {
  ...
  _scaleUpdate(ScaleUpdateDetails details) {
    ...
    controller.drag(diff.dx, diff.dy); //드레그한 위지의 xy좌표
    setState(() {}); //값 초기화
  }

  Widget _buildMarkerWidget(Offset offset, {Color color = Colors.deepOrange}){
    return Positioned(
      left: offset.dx,
      top: offset.dy,
      width: 30,
  ...
  Widget build(BuildContext context) {
    return MapLayoutBuilder(
      ...
      builder:(BuildContext context, MapTransformer transformer) {
        ...
        final middleOnScreen =
          Offset(size.width / 2, size.height / 2);

        final myLocationOnMap = transformer.toLatLng(middleOnScreen);

        final myLocationWidget = 
          _buildMarkerWidget(middleOnScreen, color: Colors.deepOrange);

        return FutureBuilder<List<ItemModel>>(
          future: ItemService().getNearByItems(widget._userModel.userKey, myLocationOnMap),
